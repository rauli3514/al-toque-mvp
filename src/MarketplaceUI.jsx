...
export default function MarketplaceUI() {
  ...
  const loadBaseData = async () => {
    ...
    let uniqueCats = [];
    if (catsData) {
      const catNames = [...new Set(catsData.map(c => c.name))];
      uniqueCats = catNames.map(name => {
    ...
    // 3. Extraer productos destacados (Promos)
    const { data: promosData } = await supabase
      .from('products')
      .select('*, businesses!inner(name, slug, business_type)')
      .eq('is_upsell_target', true)
      .eq('available', true)
      .neq('businesses.business_type', 'BAR')
      .order('created_at', { ascending: false });
  ...
  const handleSearch = (query, categoryName = null) => {
    ...
    shops.forEach(shop => {
      ...
      if (categoryName) {
        ...
        // La tienda matchea si incluye al menos uno de los tokens principales o todos
  ...
  if (loading) {
    return (
      <div style={{ display:'flex', alignItems:'center', justifyContent:'center', height:'100vh', background:'#F8FAFC', color:'#38BDF8', fontFamily:"'Inter', sans-serif" }}>
        ...
        <span className="loader">Cargando experiencias...</span>
      </div>
    );
  }
    ...
    <div style={{ minHeight:'100vh', background:'#F8FAFC', color:'#0F172A', fontFamily:"'Inter', sans-serif", paddingBottom:'90px', overflowX:'hidden' }}>
        ...
        {`
          * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
          ...
          .compact-shop-card {
            ...
            box-shadow: 0 2px 8px rgba(0,0,0,0.02); height: 88px;
          }
          .compact-shop-card:active { transform: scale(0.98); background: #F8FAFC; }
        ...
        <div style={{ maxWidth: '600px', margin: '0 auto' }}>
          ...
          <div className="search-input-wrapper">
            ...
            <input
              ...
              placeholder="Buscar pizza, perfumería, regalos..."
              value={searchQuery}
      ...
      <div style={{ maxWidth: '600px', margin: '0 auto' }}>
        ...
        {isShowingResults ? (
          ...
          <div>
            {/* CATEGORÍAS REALES EXTRAÍDAS DE SUPABASE */}
            ...
            {/* CAROUSEL PROMOS REALES DE SUPABASE */}
            {promos.length > 0 && (
            ...
            {/* COMERCIOS DESTACADOS COMPACTOS */}
